function params = STASElectric_IEA10MW ()
%
% Version:        Changes:
% --------        -------------
...
% ---------------------------------- PMSG --------------------------------------
np    = 200;                   % np    (-)     Number of poles.
Ig0   = 2968.4;                % (From gendes.m)
Lg0   = 0.005782;              % (From gendes.m)
Ig    = [0 1 2 3]*Ig0;         % Ig    (A)     RMS phase currents for nonlinear inductance.
Lg    = [1 1 1 1]*Lg0;         % Lg    (H)     Nonlinear phase inductances.
Rg    = 0.02;                  % Rg    (Ohms)  Phase resistance.
lamr  = 19.496;                % lamr  (Wb)    d-component flux linkage per phase.
% --------------------------------- DC Link ------------------------------------
...
weh   = 50*2*pi;               % weh   (rad/s) Reference network frequency.
...
Lte   = 1.1*Lt;                    % Lt    (H)     Estimated Lp + (a^2)Ls nonlinear phase inductances.
